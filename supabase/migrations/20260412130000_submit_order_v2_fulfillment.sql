-- Update submit_order_v2 to accept and persist the canonical fulfillment
-- fields: fulfillment_type, pickup_store_id, delivery_zone_code.
--
-- Backward compatibility:
--   Old callers that omit these keys produce rows with fulfillment_type = NULL
--   (legacy).  All existing behavior is preserved byte-for-byte for that path.
--
-- No inference from legacy delivery_method is performed.
--   Rationale: delivery_method is a localized Russian string.  Mapping it
--   automatically is fragile and would silently populate canonical fields
--   that old callers never intended.  Legacy rows stay legacy.
...
BEGIN
  ...
  -- ----------------------------------------------------------------
  -- 1b. Extract and validate fulfillment fields
  -- ----------------------------------------------------------------
